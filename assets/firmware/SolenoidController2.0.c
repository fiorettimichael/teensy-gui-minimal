...
void setup() {
  ...
  pinMode(currentSettings.pwm_output_pin, OUTPUT);
  analogWriteResolution(currentSettings.pwm_depth);
  analogWriteFrequency(currentSettings.pwm_output_pin, currentSettings.pwm_frequency);
  analogReadResolution(currentSettings.pwm_adc_resolution);

  // Set current sensing pin to INPUT
  pinMode(currentSettings.current_sensing_pin, INPUT);

  current_duty = (1u << currentSettings.pwm_depth) - 1;
  digitalWrite(currentSettings.pwm_output_pin, 1);
}

// === MAIN LOOP ===
void loop() {

  if (stream_enabled) handleStreaming();
...
// === SETTINGS MANAGEMENT ===
void loadSettings() {
  //EEPROM.get(SETTINGS_EEPROM_ADDR, currentSettings);
  //if (currentSettings.settings_version != SETTINGS_MAGIC) {
    setDefaultSettings();
   //saveSettings();
  //}
}
...
void saveSettings() {
  //currentSettings.settings_version = SETTINGS_MAGIC;
  //EEPROM.put(SETTINGS_EEPROM_ADDR, currentSettings);
}

void setDefaultSettings() {
  currentSettings.pwm_output_pin      = 5;
  currentSettings.pwm_sensing_pin     = A6;
  currentSettings.current_sensing_pin = A0;
  currentSettings.pwm_frequency       = 10000;
  currentSettings.pwm_adc_rate        = 10000;
  currentSettings.current_adc_rate    = 10000;
  currentSettings.pwm_adc_resolution  = 10;
  currentSettings.current_adc_resolution = 10;
  currentSettings.pwm_depth           = 10;
}
...
void handleCommand(uint8_t* data, uint8_t len) {
  ...
  switch (cmd) {
    ...
    case CMD_SET_PWM_OUTPUT_PIN:
      if (l == 1) {
        currentSettings.pwm_output_pin = p[0];
    ...
    case CMD_SET_CURRENT_SENSING_PIN:
      if (l == 1) {
        currentSettings.current_sensing_pin = p[0];
        pinMode(currentSettings.current_sensing_pin, INPUT); // Set new pin to INPUT
    ...
    case CMD_SET_PWM_FREQ:
      if (l == 4) {
        ...
        currentSettings.pwm_frequency = freq;
        analogWriteFrequency(currentSettings.pwm_output_pin, freq);
        sendAck(CMD_SET_PWM_FREQ);
      } else sendError(cmd, ERR_INVALID_PAYLOAD);
      break;

    // … repeat same ACK‐on‐success pattern for each setter …

    case CMD_SET_DUTY_ACK:
      if (l == 2) {
        uint16_t d = toUInt16(p);
        uint16_t maxD = (1u << currentSettings.pwm_depth) - 1;
        if (d > maxD) sendError(cmd, ERR_INVALID_DUTY);
        else {
          current_duty = d;
          analogWrite(currentSettings.pwm_output_pin, d);
...
void sendStreamPacket() {
  uint8_t packet[1 + 1 + 2 * STREAM_BUFFER_SIZE + 1];
  packet[0] = STREAM_PACKET_MAGIC;
  packet[1] = 0; // flags (optional)
  for (int i = 0; i < STREAM_BUFFER_SIZE; ++i) {
    uint16_t s = sample_buffer[i];
    packet[2 + 2 * i] = s & 0xFF;
    packet[3 + 2 * i] = s >> 8;
  }
  uint8_t crc = computeCRC8(&packet[1], 1 + 2 * STREAM_BUFFER_SIZE);
  packet[1 + 1 + 2 * STREAM_BUFFER_SIZE] = crc;
...
void handleStreaming() {
  ...
  if (sample_interval_us == 0) {
    sample_interval_us = 1000000UL / currentSettings.current_adc_rate;
  ...
  if ((now - last_sample_time) >= sample_interval_us) {
    ...
    sample_buffer[sample_index++] = analogRead(currentSettings.current_sensing_pin);
...
void sendStatusPacket() {
  ...
  payload[idx++] = currentSettings.pwm_output_pin;
  payload[idx++] = currentSettings.pwm_sensing_pin;
  payload[idx++] = currentSettings.current_sensing_pin;
  payload[idx++] = currentSettings.pwm_frequency >> 24;
  payload[idx++] = currentSettings.pwm_frequency >> 16;
  payload[idx++] = currentSettings.pwm_frequency >> 8;
  payload[idx++] = currentSettings.pwm_frequency;
  payload[idx++] = currentSettings.pwm_adc_rate >> 8;
  payload[idx++] = currentSettings.pwm_adc_rate;
  payload[idx++] = currentSettings.current_adc_rate >> 8;
  payload[idx++] = currentSettings.current_adc_rate;
  payload[idx++] = currentSettings.pwm_adc_resolution;
  payload[idx++] = currentSettings.current_adc_resolution;
  payload[idx++] = currentSettings.pwm_depth;
...
// Teensy Solenoid Controller Firmware (Refactored)
// Features:
// - Interrupt-based control loop at 10–20 kHz
// - Real-time duty + current sensing streaming
// - Automation trajectory engine with segment buffer
// - Fast interpolation (step or linear)
...
// === STRUCTS ===
struct Settings {
  uint8_t pwm_output_pin = 5;
  uint8_t current_sensing_pin = A0;
  uint32_t pwm_frequency = 10000;
  uint8_t pwm_depth = 10;
  uint8_t current_adc_resolution = 10;
  uint16_t current_adc_rate = 10000;
};
...
inline void startNextSegment() {
  ...
  TrajectorySegment seg = traj_buffer[traj_tail];
...
FASTRUN void controlISR() {
  ...
  SamplePair& s = stream_buffer[stream_index++];
  s.duty = next_duty;
  s.current = current;
...
void handleCommand(uint8_t* data, uint8_t len) {
  ...
  switch (cmd) {
    ...
    case CMD_QUEUE_TRAJ_SEG:
      if (len >= 8) {
        TrajectorySegment& s = traj_buffer[traj_head];
        s.start = (p[0] << 8) | p[1];
        s.end = (p[2] << 8) | p[3];
...
void setup() {
  ...
  analogReadResolution(cfg.current_adc_resolution);
